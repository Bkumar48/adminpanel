...
const EditRule = () => {
    ...
    useEffect(() => {
        const fetchRuleData = async () => {
            ...
            if (res.status === 200) {
                // setRuleData(res.data.priceRules);
                setTitle(res.data.priceRules.title);
                setProducts(res.data.priceRules.product_id);
                setRows(res.data.priceRules.variants);
                console.log(res.data.priceRules)
            } else {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            const res = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/v1/product/allProduct`, {
                headers: {
                    Authorization: `Bearer ${sessionStorage.getItem("token")}`,
                },
            });
            if (res.status === 200) {
                setAllProducts(res.data.proData);
            } else {
                enqueueSnackbar("Something went wrong", {
                    variant: "error",
                });
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const res = await axios.put(`${process.env.REACT_APP_BASE_URL}/api/v1/admin/pricerules/${id}`, {
            title: title,
            product_id: products,
            variants: rows,
        }, {
